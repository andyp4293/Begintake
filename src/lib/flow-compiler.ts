...
export function compileFlowToPrompt(flow: FlowData, assistantName?: string, firmName?: string): string {
  ...
  sections.push('Whenever the caller clearly provides their name, callback number, email, whether they are new or existing, whether they are calling for themselves or someone else, or their core issue, silently call captureIntakeState with every slot you now know.');
  ...
  sections.push('  Do NOT promise an immediate live handoff.');
  sections.push('  Say you will send their information to the right team member or lawyer and they will reach out as soon as possible.');
  sections.push('  Continue collecting any missing basic contact information you still need, then finish with the appropriate follow-up section.');
  sections.push('');
